...
public class NeuralNetwork
{
    ...
    /// <summary>Gets a read-only dictionary of all nodes in the network, keyed by their ID.</summary>
    [JsonIgnore]
    public IReadOnlyDictionary<int, NNNode> Nodes => _nodes;
    
    /// <summary>Gets a read-only list of all connections in the network.</summary>
    [JsonIgnore]
    public IReadOnlyList<NNConnection> Connections => _connections;

    /// <summary>Gets a read-only list of the network's input nodes.</summary>
    [JsonIgnore]
    public IReadOnlyList<NNNode> InputNodes => _inputNodes;

    /// <summary>Gets a read-only list of the network's output nodes.</summary>
    [JsonIgnore]
    ...
    public void AddConnection(NNConnection connection)
    {
        ...
        {
            var errorMsg = $"AddConnection failed: Node(s) not found for connection {connection.FromNodeId}->{connection.ToNodeId}.";
        ...
        _sortedNodes = null; // Invalidate cache.
    ...
    public void Evaluate(IReadOnlyList<float> inputs)
    {
        ...
        {
            var errorMsg = $"Evaluation failed: Mismatched input count. Expected {_inputNodes.Count}, got {inputs.Count}.";
        ...
        // Initialize all nodes to their bias.
        foreach (var node in _nodes.Values)
        {
        ...
        // Propagate values through the topologically sorted graph.
        foreach (var node in _sortedNodes)
        {
            float activatedValue = node.NodeType == NNNodeType.Input
                ? node.Value
                : node.ActivationFunction switch
                {
                    ...
                    _ => (float)Math.Tanh(node.Value), // Tanh is the default
                };
    ...
    private List<NNNode> TopologicalSort()
    {
        ...
        {
            const string errorMsg = "Cycle detected in neural network graph! Cannot evaluate.";
            Logger.Log("NN_BRAIN", LogLevel.Error, errorMsg);
