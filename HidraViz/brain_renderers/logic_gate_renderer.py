...
class LogicGateRenderer(BaseBrainRenderer):
    ...
    # Map for Integer fallbacks (Legacy support)
    INT_TO_NAME_MAP = { 
        0: "BUFFER", 1: "NOT", 2: "AND", 3: "OR", 
    ...
    def _resolve_gate_name(self, raw_type) -> str:
        """Normalizes API enum values (String or Int) to a standard uppercase string."""
        if isinstance(raw_type, str):
            return raw_type.upper()
        return self.INT_TO_NAME_MAP.get(raw_type, "BUFFER")

    def _evaluate_combinational(self, gate_name, inputs, threshold):
        ...
        if len(b_inputs) == 1:
            return not b_inputs[0] if gate_name in ["NOT", "NAND", "NOR"] else b_inputs[0]

        if gate_name == "AND": return all(b_inputs)
        if gate_name == "OR": return any(b_inputs)
        if gate_name == "NAND": return not all(b_inputs)
        if gate_name == "NOR": return not any(b_inputs)
        if gate_name == "XOR": return reduce(operator.xor, b_inputs)
        if gate_name == "XNOR": return not reduce(operator.xor, b_inputs)
        return b_inputs[0] # Default to Buffer behavior
    ...
    def prepare_animation_state(self, brain_data: dict, initial_value: float) -> dict | None:
        ...
        raw_type = brain_data.get('gateType', "AND")
        gate_name = self._resolve_gate_name(raw_type)
        threshold = brain_data.get('threshold', 0.5)
        
        # For visualization, we assume the single input value applies to all gate inputs
        num_inputs = 2 
        inputs = [initial_value] * num_inputs
        
        output_bool = self._evaluate_combinational(gate_name, inputs, threshold)
    ...
    def render(self, painter: QPainter, data: dict, rect: QRect, animation_state: dict | None = None):
        ...
        raw_gate_type = data.get('gateType', "AND")
        
        if is_ff:
            # Handle FlipFlop String Enum
            raw_ff = data.get('flipFlop')
            ff_label = str(raw_ff).upper() if raw_ff else "FF"
            label = f"{ff_label} Flip-Flop"
        else:
            label = self._resolve_gate_name(raw_gate_type)
