...
from .logging import LoggingClient  # <-- ADDED: Import the new client
...
class HidraApiException(Exception):
    ...
    Attributes:
        ...
        error_type (str): The 'error' or 'title' field from the JSON response.
        message (str): The 'message' or 'detail' field from the JSON response.
    """
...
class HidraApiClient:
    ...
    This class handles the underlying HTTP communication and provides access
    to clients for specific API controllers through its properties.
    """
    def __init__(self, base_url: str = "http://localhost:5000"):
        ...
        # --- Controller Clients ---
        # Each property provides access to a specific controller's client.
        ...
        self.logging = LoggingClient(self)  # <-- ADDED: Instantiate the new client
    ...
    def _request(self, method: str, path: str, **kwargs) -> Any:
        ...
        This is used by the controller-specific client classes.
        
        Raises:
            HidraApiException: If the API returns a non-successful status code.
        """
        ...
        try:
            ...
            if not response.ok:
                try:
                    ...
                    # Check for standard custom error format first
                    ...
                    # If the error response is not JSON, raise a generic exception
                    response.raise_for_status()

            # Handle success responses that might not have a body (e.g., 204 No Content)
    ...
    def _request_text(self, method: str, path: str, **kwargs) -> str:
        """
        Internal helper for making API requests that return plain text.
        """
        url = f"{self.base_url}/{path}"
        try:
            # For text requests, we don't need to specify JSON content/accept types.
            # We create a temporary header dict for this request.
            headers = self.session.headers.copy()
            headers.pop("Content-Type", None)
            headers.pop("Accept", None) # Server will default to its content type
            
            response = self.session.request(method, url, headers=headers, **kwargs)

            if not response.ok:
                # The success response is text, but the error might still be JSON.
                try:
                    error_data = response.json()
                    ...
                    response.raise_for_status() # Fallback to standard HTTP error
